<template>
  <div class="identity-card">
    <template v-if="!isLoggedIn">
      <label>
        管理员用户名
        <input v-model.trim="identityForm.username" placeholder="请输入管理员用户名" />
      </label>
      <label>
        登录密码
        <input
      ...
      <button class="button ghost" type="button" :disabled="loading" @click="emit('login')">
        登录管理台
      </button>
    </template>
    ...
    <template v-else>
      ...
      <label>
        目标玩家用户 ID
        <input
          v-model.trim="identityForm.targetUserId"
          placeholder="要查询或发令牌的玩家 UUID"
        />
      </label>
