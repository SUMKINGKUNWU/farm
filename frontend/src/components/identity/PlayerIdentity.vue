<template>
  <div class="identity-card">
    <template v-if="!isLoggedIn">
      <label>
        玩家用户名
        <input v-model.trim="identityForm.username" placeholder="例如 farmer_001" />
      </label>
      <label>
        玩家昵称
        <input v-model.trim="identityForm.nickname" placeholder="注册时填写显示名" />
      </label>
      <label>
        登录密码
        <input
      ...
      <div class="button-row">
        ...
        <button class="button subtle" type="button" :disabled="loading" @click="emit('register')">
          注册并登录
        </button>
      </div>
    </template>
    ...
    <template v-else>
      ...
      <button class="button ghost" type="button" :disabled="loading" @click="emit('refresh-dashboard')">
        刷新农场
      </button>
